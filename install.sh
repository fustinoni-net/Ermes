...
function set_lighttpd_config_file() {
...
function configure_lighttpd(){
    ...
    cat ${LIGHTTPD_CONF_DIR}lighttpd.conf.tmpl | envsubst '$AP_SSID' > /etc/lighttpd/lighttpd.conf

    cp -R ${LIGHTTPD_CONF_DIR}connectivitycheck.gstatic.com ${WEBROOT_DIR}
    cp -R ${LIGHTTPD_CONF_DIR}jail ${WEBROOT_DIR}
